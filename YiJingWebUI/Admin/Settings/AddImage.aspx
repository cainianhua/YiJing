<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/MasterPages/Admin.Master" AutoEventWireup="true" CodeBehind="AddImage.aspx.cs" Inherits="YiJingWebUI.Admin.Settings.EditLogo" %>
        ...
        <div class="tab-content default-tab" id="tab1">
            ...
            <fieldset>
				<p>
					...
					<asp:RequiredFieldValidator ID="RequiredFieldValidator1" ControlToValidate="TextValue" Display="Dynamic" runat="server" CssClass="input-notification error png_bg" ErrorMessage="请上传新的LOGO"></asp:RequiredFieldValidator>
				</p>
				<p>
					<label>说明：</label>
                ...
                <p>
					...
					<asp:Button ID="btnSave" runat="server" Text="保存LOGO" CssClass="button" onclick="btnSave_Click" />
                </p>
            </fieldset>
            </form>
        </div>
		...
		jQuery(function ($) {
			...
			$('#file_upload').uploadify("settings", "onUploadSuccess", function (file, data, response) {
				...
				if (returns.length == 2 && returns[0] == "success") {
					$("img[id$=imgTextValue]").attr("src", returns[1]);
					$("input[id$=TextValue]").val(returns[1]);
